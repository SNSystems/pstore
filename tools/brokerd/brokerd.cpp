...
namespace pstore {
    namespace broker {
        int run_broker (switches const & opt) {
            ...
            brokerface::fifo_path fifo{opt.pipe_path.has_value () ? opt.pipe_path->c_str ()
                                                                  : nullptr};
            ...
            log (priority::notice, "starting threads");
            {
                ...
                quit = create_quit_thread (make_weak (commands), make_weak (scav),
                                           opt.num_read_threads, &http_status, &uptime_done);
